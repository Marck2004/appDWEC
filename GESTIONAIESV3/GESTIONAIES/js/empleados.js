    ...
    lista.forEach(empleado => {
        ...
        for (propiedad in empleado) {
            ...
            if (propiedad != "foto") {
                ...
                celda.addEventListener("click", () => mostrarDiv());
            }
        }
        var celda = document.createElement("td");
        fila.appendChild(celda);
...
function ordenar() {
    ...
    function mostrarDiv(dni){

        let crearDiv = document.getElementById('EditarDatos');

        let nuevoNombre = `<br>DNI: <input type=text id='DNI'><br>
        Nombre: <input type=text id='nombre'><br>
    Apellido: <input type=text id='apellido'><br>
    Edad: <input type=number id='edad'><br>
    Antiguedad: <input type=number id='antiguo'><br>
    Puesto: <input type=text id='puesto'><br>
    <input type=submit value=Enviar id='enviar'>`;

        crearDiv.innerHTML = nuevoNombre;
        
        var posicion = listaEmpleados.findIndex((empleado) => empleado.DNI == dni);
